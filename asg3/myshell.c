...
int isExist(const char *a,char c){
	int i=0;
	while(a[i]!='\0'){
		if(a[i]==c){
			return 1;
...
int isPipingError(char *cmd){
	int pipeCount = isPipingNeeded(cmd);
	if(pipeCount == 0){
		return 0;
	}
	...
	if(isExist(process[0],'>')==1){
		printf("\nOutput Redirection not possible in the 1st process while piping !!");
		return 1;
	}
	for(int i=1;i<processCount-1;i++){
		// printf("\nPROCESS[%d] = {%s}",i,process[i]);
		clearTabs(process[i]);
		if(isExist(process[i],'<')==1||isExist(process[i],'>')==1){
			printf("\nIn between IO redirection not possible in piping except 1st and last process !!");
			return 1;
		}
	}
	if(isExist(process[processCount-1],'<')==1){
		printf("\nInput Redirection not possible in the last process while piping !!");
		return 1;
	}
	// printf("\n");
	for(int i=0;i<processCount;i++){
...
void runshell(){
    ...
    while(1){
        ...
        printf("\nINPFLAG = %d OUTPUTFLAG = %d",inp_flag,out_flag);
