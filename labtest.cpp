...
void getSemaphores(int *full,int *empty,int *mutex,int *shmid,int *mutex2){
	int full_,empty_,mutex_,shmid_,mutex2_;
...
void initSharedBufferAndSemaphores(int full,int empty,int mutex,int shmid,int mutex2){
...
void produceItem(int shmid,int item){
...
void producer(int shmid,int full,int empty,int mutex,int mutex2){
	int item;
	...
	while(1){
		...
		item = getItem();
		P(mutex2,semP);
		if(isBufferFull(shmid) == 1){
			printf("\nProducer inserts item : Buffer FULL!\nProducer waits\n");
		}
		V(mutex2,semV);
		P(empty,semP);
		P(mutex,semP);
		produceItem(shmid,item);
		V(mutex,semV);
		V(full,semV);
	}
	return;
...
void consumer(int shmid,int full,int empty,int mutex,int mutex2){
	struct sembuf semV,semP;
	initSemaphoreOperation(&semV,0,0,1);
	initSemaphoreOperation(&semP,0,0,-1);
	while(1){
...
void runConsumerAndCreateProducer(int full,int empty,int mutex,int shmid,int mutex2){
	int prodpid = fork();
	if(prodpid == 0){
		// child process process
		// need to run child code
		srand(time(NULL));
		producer(shmid,full,empty,mutex,mutex2);
		exit(0);
	}else{
		// need to run parent code
		sleep(1);
		srand(time(NULL));
		consumer(shmid,full,empty,mutex,mutex2);
		return;
	}
}
...
int main(){
	...
	system("reset");
	int full,empty,mutex,shmid,mutex2;
	getSemaphores(&full,&empty,&mutex,&shmid,&mutex2); // all semaphores and shared memories creatred
	initSharedBufferAndSemaphores(full,empty,mutex,shmid,mutex2);
	runConsumerAndCreateProducer(full,empty,mutex,shmid,mutex2);
	return 0;
}